...
object Main {
  def main(args: Array[String]) {
    ...
    val climateRelatedNewsCount = spark.sql("SELECT COUNT(*) FROM news WHERE containsWordGlobalWarming = true")
    val nonClimateRelatedNewsCount = spark.sql("SELECT COUNT(*) FROM news WHERE containsWordGlobalWarming = false")
    ...
    climateRelatedNewsCount.show()
    nonClimateRelatedNewsCount.show()

    enrichedDataset.write.partitionBy("date", "media").parquet("./output")
